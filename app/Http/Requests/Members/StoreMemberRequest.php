...
class StoreMemberRequest extends FormRequest
{
            ...
            'church_id' => [
                ...
                function ($attribute, $value, $fail) use ($currentUser) {
                    if ($value && !$currentUser->canAccessChurch((int) $value)) {
                        $fail('Você não possui autorização para cadastrar membros nesta congregação.');
                    }
                },
            ],
            'congregation_id' => [
    ...
    {
        return [
            'church_id.required' => 'Selecione a congregação à qual o membro pertencerá.',
     ...
     * Protege contra upload de arquivos PHP disfarados como imagens.
     */
