...
@section('header', 'Editar Congregação')
...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-xl font-bold text-white tracking-tight">Editar: {{ $church->name }}</h1>
            <p class="text-xs text-slate-400 mt-1">Atualize os dados cadastrais, endereço e dirigentes responsáveis.</p>
        </div>
        <div class="flex items-center gap-2">
            <a href="{{ route('churches.show', $church) }}" class="px-3.5 py-2 bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs font-semibold rounded-xl border border-slate-700 transition-colors">
                Ver Ficha
            </a>
    ...
    <!-- Formulário -->
    <form method="POST" action="{{ route('churches.update', $church) }}" enctype="multipart/form-data" class="bg-slate-900/60 p-6 md:p-8 rounded-2xl border border-slate-800/80 space-y-8 shadow-xl">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <input type="text" name="code" id="code" value="{{ old('code', $church->code) }}"
                        class="w-full px-3.5 py-2.5 bg-slate-950/80 border border-slate-800 rounded-xl text-xs text-slate-100 placeholder-slate-500 focus:outline-none focus:ring-1 focus:ring-brand-500">
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="cnpj" id="cnpj" value="{{ old('cnpj', $church->cnpj) }}"
                        class="w-full px-3.5 py-2.5 bg-slate-950/80 border border-slate-800 rounded-xl text-xs text-slate-100 placeholder-slate-500 focus:outline-none focus:ring-1 focus:ring-brand-500">
                </div>
        ...
        <div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div>
                    ...
                    <input type="text" name="zip_code" id="zip_code" value="{{ old('zip_code', $church->zip_code) }}"
                        class="w-full px-3.5 py-2.5 bg-slate-950/80 border border-slate-800 rounded-xl text-xs text-slate-100 placeholder-slate-500 focus:outline-none focus:ring-1 focus:ring-brand-500">
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="number" id="number" value="{{ old('number', $church->number) }}"
                        class="w-full px-3.5 py-2.5 bg-slate-950/80 border border-slate-800 rounded-xl text-xs text-slate-100 placeholder-slate-500 focus:outline-none focus:ring-1 focus:ring-brand-500">
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="complement" id="complement" value="{{ old('complement', $church->complement) }}"
                        class="w-full px-3.5 py-2.5 bg-slate-950/80 border border-slate-800 rounded-xl text-xs text-slate-100 placeholder-slate-500 focus:outline-none focus:ring-1 focus:ring-brand-500">
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="neighborhood" id="neighborhood" value="{{ old('neighborhood', $church->neighborhood) }}"
                        class="w-full px-3.5 py-2.5 bg-slate-950/80 border border-slate-800 rounded-xl text-xs text-slate-100 placeholder-slate-500 focus:outline-none focus:ring-1 focus:ring-brand-500">
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="state" id="state" value="{{ old('state', $church->state) }}" required maxlength="2"
        ...
        <div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                ...
                <div class="sm:col-span-2">
                    ...
                    <input type="text" name="responsible_name" id="responsible_name" value="{{ old('responsible_name', $church->responsible_name) }}"
                        class="w-full px-3.5 py-2.5 bg-slate-950/80 border border-slate-800 rounded-xl text-xs text-slate-100 placeholder-slate-500 focus:outline-none focus:ring-1 focus:ring-brand-500">
                </div>
                ...
                <div>
                    ...
                    <input type="date" name="foundation_date" id="foundation_date" value="{{ old('foundation_date', $church->foundation_date?->format('Y-m-d')) }}"
                ...
                <div class="sm:col-span-1">
                    ...
                    <select name="status" id="status" required class="w-full px-3.5 py-2.5 bg-slate-950/80 border border-slate-800 rounded-xl text-xs text-slate-100 focus:outline-none focus:ring-1 focus:ring-brand-500">
                        <option value="active" {{ old('status', $church->status) === 'active' ? 'selected' : '' }}>Ativa</option>
                        <option value="inactive" {{ old('status', $church->status) === 'inactive' ? 'selected' : '' }}>Inativa</option>
                    </select>
                </div>
                ...
                <div class="sm:col-span-full">
                    ...
                    <textarea name="notes" id="notes" rows="3"
                        class="w-full px-3.5 py-2.5 bg-slate-950/80 border border-slate-800 rounded-xl text-xs text-slate-100 placeholder-slate-500 focus:outline-none focus:ring-1 focus:ring-brand-500">{{ old('notes', $church->notes) }}</textarea>
                </div>
            </div>
        </div>
        ...
        <div class="flex items-center justify-end gap-3 pt-4 border-t border-slate-800">
            <a href="{{ route('churches.show', $church) }}" class="px-4 py-2.5 bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs font-semibold rounded-xl border border-slate-700/60 transition-colors">
                Cancelar
            </a>
